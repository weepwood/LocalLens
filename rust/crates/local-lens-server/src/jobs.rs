...
use local_lens_core::{MediaItem, MetadataJob, ThumbnailJob, random_id};
use tokio::{
    process::Command,
    time::{Duration, sleep},
};
    ...
    match result {
        ...
        Err(error) => {
            ...
            if let Err(store_error) = state
                .store
                .finish_thumbnail_job(&job, status, &message)
                .await
            {
            ...
            if status == "pending" {
                ...
                tracing::warn!(worker = number, media = %job.media_id, %error, "缩略图任务失败");
            }
        }
    }
}
...
async fn process_metadata_job(state: &AppState, job: MetadataJob, number: usize) {
    ...
    match result {
        ...
        Err(error) => {
            ...
            if let Err(store_error) = state
                .store
                .finish_metadata_job(&job, status, &message)
                .await
            {
            ...
            if status == "pending" {
                ...
                tracing::warn!(worker = number, media = %job.media_id, %error, "元数据任务失败");
            }
        }
    }
}
...
pub async fn generate_thumbnail(
    ...
    if media.media_type == "image" {
        ...
        let native_result = tokio::task::spawn_blocking(move || {
            generate_native_image_thumbnail(&native_source, &native_target, width)
        })
        .await
        .context("图片缩略图线程异常退出")?;
...
pub fn thumbnail_path(state: &AppState, media: &MediaItem, width: i64) -> PathBuf {
    let timestamp = DateTime::parse_from_rfc3339(&media.modified_at)
...
async fn generate_ffmpeg_thumbnail(
    ...
    if media.media_type == "video" {
        command.args([
            "-ss",
            if media.duration_ms > 0 && media.duration_ms < 6000 {
                "0.2"
            } else {
                "2"
            },
